      ...
      return props.categories.map((n,i) => {
        ...
        return arr.map((m,j) => {
          if(m.title) {
            ...
            return (
              <Item k={m.names.name1+m.names.surname1+j} surnames={m.names.surname1 + " " + m.names.surname2} names={m.names.name1 + " " + m.names.name2} action={props.action} href={props.href}/>
            )
          }
        })
      })
